...
static int __init xilaxitimer_init(void)  
{
  ...
  if (!request_mem_region(XIL_AXI_TIMER_BASEADDR, XIL_AXI_TIMER_HIGHADDR - XIL_AXI_TIMER_BASEADDR + 1, DEVICE_NAME)) {
     printk(KERN_ALERT "xilaxitimer_init: Could not request memory region\n");
     return -ENXIO;
  }
  else {
  ...
  //dev_virtaddr = ioremap(XIL_AXI_TIMER_BASEADDR,
//				 XIL_AXI_TIMER_HIGHADDR - XIL_AXI_TIMER_BASEADDR + 1);

  printk("xilaxitimer_init: Remaped IO area, no cache. Timer mapped at:0x%08X\n", (unsigned int)dev_virtaddr);
  /* 
   * Set Timer Counter
   */
  //iowrite32(TIMER_CNT,
//	    dev_virtaddr + XIL_AXI_TIMER_TLR_OFFSET);
  iowrite32(TIMER_CNT,
	    dev_virtaddr);
  printk("xilaxitimer_init: Successfully written to timer reg offset 0\n");
  data = ioread32(dev_virtaddr);
  printk("xilaxitimer_init: Successfully read from timer reg offset 0\n");
  iowrite32(TIMER_CNT,
	    dev_virtaddr + XIL_AXI_TIMER_TLR_OFFSET);
  printk("xilaxitimer_init: Successfully written to timer reg offset 4\n");
